...
void print_board(char *board, string player, bool test) {
    cout << player << (player.back() == 'S' || player.back() == 's' ? "' board." : "'s board.") << endl;
    int healthy = 0, hit = 0;
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            if (board[i * cols + j] == 'A' || board[i * cols + j] == 'B' || board[i * cols + j] == 'C' ||
                board[i * cols + j] == 'D' || board[i * cols + j] == 'M' || board[i * cols + j] == 'S') {
                healthy++;
            }
            if (board[i * cols + j] == '*') {
                hit++;
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            ...
            else if (board[i * cols + j] == 'O' || board[i * cols + j] == 'X' || board[i * cols + j] == '*') {
                cout << setw(width) << board[i * cols + j];
            }
            else {
                cout << setw(width) << 'O';
...
bool add_ship(char *board, bool *board_check, int beg_row, int end_row, int beg_col, int end_col, int n, char c) {
    int counter = 0;
    if (beg_row == end_row) {
...
void add_ship_random(char *board, bool *board_check, int i, int n, string ship) {
    ...
    while (add_ship(board, board_check, beg_row, end_row, beg_col, end_col, n, ship[0]));
}

void add_ship_user(string player, int i, int ships, char *board, bool *board_check, int n, string ship) {
    print_board(board, player, true);
    cout << "Place " << i << ". " << ship << ". Do you want random placement?" << endl;
    ...
    if (int_input == 0) {
        ...
        while (true) {
            if (n == 1) {
                ...
                cout << "Type 0 for horizontal and type any integer for vertical placement." << endl;
                cin >> int_input;
                if (int_input != 0) {
                    cout << "Enter the column number." << endl;
                    cin >> beg_col;
            ...
            if (!add_ship(board, board_check, beg_row, end_row, beg_col, end_col, n, ship[0])) {
                break;
            }
...
void add_ship_main(bool first, bool opponent, string player1, string player2, char *player1_board,
                   bool *player1_board_check, char *player2_board, bool *player2_board_check, int n, string ship) {
    if (rows >= n || cols >= n) {
        int maxShips = min(control_board_adding(player1_board_check, n), control_board_adding(player2_board_check, n));
        if (maxShips > 0) {
            int ships;
            cout << "How many " << ship << "s do you want to have? (Enter 0 if you do not want)" << endl;
            cin >> ships;
            if (ships > 0) {
                srand((unsigned) time(NULL));
                for (int i = 1; i <= ships; i++) {
                    if ((control_board_adding(player1_board_check, n) < 1) || (control_board_adding(player2_board_check, n) < 1)) {
                        cout << "There is no space left in this board for " << ship << "." << endl;
                        break;
                    }
                    if (first) {
                        add_ship_user(player1, i, ships, player1_board, player1_board_check, n, ship);
                    }
                    if (opponent) {
                        ...
                        add_ship_user(player2, i, ships, player2_board, player2_board_check, n, ship);
                    }
                    if (!first) {
                        add_ship_user(player1, i, ships, player1_board, player1_board_check, n, ship);
                    }
...
bool attack_ship(string player1, string player2, int row, int col, char *board, bool *board_check) {
...
bool attack_ship_random(string player1, string player2, char *board, bool *board_check) {
...
bool attack_ship_user(string player1, string player2, char *other_board, char *board, bool *board_check) {
    cout << player1 << " attacks." << endl;
...
int main() {
    ...
    do {
        add_ship_main(first_play_choice, opponent, player1, player2, player1_board, player1_board_check, player2_board,
                      player2_board_check, 5, "Aircraft carrier");
        add_ship_main(first_play_choice, opponent, player1, player2, player1_board, player1_board_check, player2_board,
                      player2_board_check, 4, "Cruiser");
        add_ship_main(first_play_choice, opponent, player1, player2, player1_board, player1_board_check, player2_board,
                      player2_board_check, 3, "Destroyer");
        add_ship_main(first_play_choice, opponent, player1, player2, player1_board, player1_board_check, player2_board,
                      player2_board_check, 2, "Frigate");
        add_ship_main(first_play_choice, opponent, player1, player2, player1_board, player1_board_check, player2_board,
                      player2_board_check, 1, "Boat");
    }
    while (!control_board_start(player1_board_check));
    while (control_finish(player1_board_check) && control_finish(player2_board_check)) {
        srand((unsigned) time(NULL));
        if (first_play_choice) {
            while (control_finish(player1_board_check) && control_finish(player2_board_check)) {
                if (!attack_ship_user(player1, player2, player1_board, player2_board, player2_board_check) || !continue_attack_choice) {
                    break;
                }
            }
        }
        while (control_finish(player1_board_check) && control_finish(player2_board_check)) {
            bool round;
            if (opponent) {
                round = attack_ship_random(player2, player1, player1_board, player1_board_check);
            }
            else {
                round = attack_ship_user(player2, player1, player2_board, player1_board, player1_board_check);
            }
